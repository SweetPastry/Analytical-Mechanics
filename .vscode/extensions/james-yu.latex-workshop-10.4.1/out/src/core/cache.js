...
async function updateChildrenInput(fileCache, rootPath) {
    ...
    while (true) {
        ...
        if (lw_1.lw.watcher.src.has(vscode.Uri.file(result.path))) {
            continue;
        }
        add(result.path);
...
async function updateChildrenXr(fileCache, rootPath) {
    ...
    while (true) {
        ...
        if (lw_1.lw.watcher.src.has(vscode.Uri.file(externalPath))) {
            continue;
        }
        add(externalPath);
 ...
 */
function getIncludedBib(filePath, includedBib = [], checkedTeX = []) {
