...
function getJobname(texPath) {
    const jobname = vscode.workspace.getConfiguration('latex-workshop', vscode.Uri.file(texPath)).get('latex.jobname');
...
function kpsewhich(target, isBib = false) {
    ...
    try {
        ...
        const kpsewhichReturn = lw_1.lw.external.sync(command, args, { cwd: lw_1.lw.root.dir.path || vscode.workspace.workspaceFolders?.[0].uri.path });
 ...
 * `true`.
 */
async function read(filePath, raise = false) {
    try {
