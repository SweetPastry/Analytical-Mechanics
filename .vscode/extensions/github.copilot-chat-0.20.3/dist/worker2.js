...
`)}};var ae=class{constructor(t,r){this.syntaxTreeRoot=r;this.roots=[];this.formTree(t)}formTree(t){t.sort((o,a)=>o.mainBlock.startIndex-a.mainBlock.startIndex||o.mainBlock.endIndex-a.mainBlock.endIndex);let r=[],n=()=>r[r.length-1],s=(o,a)=>o.mainBlock.startIndex===a.mainBlock.startIndex&&o.mainBlock.endIndex===a.mainBlock.endIndex;for(let o of t){let a={info:o,children:[]},_=n();if(!_){this.roots.push(a),r.push(a);continue}if(!s(_.info,o)){for(;_&&!x.doesContain(_.info.mainBlock,o.mainBlock);)r.pop(),_=n();_?_.children.push(a):this.roots.push(a),r.push(a)}}}};var X=class{constructor(t,r){this.prev=null;this.next=null;this.key=t,this.value=r}},J=class{constructor(t=10){if(t<1)throw new Error("Cache size must be at least 1");this._capacity=t,this._cache=new Map,this._head=new X("",null),this._tail=new X("",null),this._head.next=this._tail,this._tail.prev=this._head}_addNode(t){t.prev=this._head,t.next=this._head.next,this._head.next.prev=t,this._head.next=t}_removeNode(t){let r=t.prev,n=t.next;r.next=n,n.prev=r}_moveToHead(t){this._removeNode(t),this._addNode(t)}_popTail(){let t=this._tail.prev;return this._removeNode(t),t}clear(){this._cache.clear(),this._head.next=this._tail,this._tail.prev=this._head}deleteKey(t){let r=this._cache.get(t);if(r)return this._removeNode(r),this._cache.delete(t),r.value}get(t){let r=this._cache.get(t);if(r)return this._moveToHead(r),r.value}keys(){let t=[],r=this._head.next;for(;r!==this._tail;)t.push(r.key),r=r.next;return t}getValues(){let t=[],r=this._head.next;for(;r!==this._tail;)t.push(r.value),r=r.next;return t}put(t,r){let n=this._cache.get(t);if(n)n.value=r,this._moveToHead(n);else if(n=new X(t,r),this._cache.set(t,n),this._addNode(n),this._cache.size>this._capacity){let s=this._popTail();return this._cache.delete(s.key),[s.key,s.value]}}},_e=class{constructor(t){this.actual=new J(t)}dispose(){this.clear()}clear(){let t=this.actual.getValues();for(let r of t)r.dispose();this.actual.clear()}deleteKey(t){let r=this.actual.deleteKey(t);r&&r.dispose()}get(t){return this.actual.get(t)}keys(){return this.actual.keys()}getValues(){return this.actual.getValues()}put(t,r){let n=this.actual.put(t,r);n&&n[1].dispose()}};var ee=gt(require("path")),yt=j(),de=class{constructor(){this.loadedLanguagesCache=new Map}loadLanguage(t){return this.loadedLanguagesCache.has(t)||this.loadedLanguagesCache.set(t,this._doLoadLanguage(t)),this.loadedLanguagesCache.get(t)}_doLoadLanguage(t){let n=`tree-sitter-${t==="csharp"?"c-sharp":t}.wasm`,s=ee.basename(__dirname)==="dist"?ee.resolve(__dirname,n):ee.resolve(__dirname,"../../../../dist",n);return yt.Language.load(s)}};var Ne=j(),le=class e{static{this.INSTANCE=new e}static{this.CACHE_SIZE_PER_LANGUAGE=5}constructor(){this.caches=new Map,this.languageLoader=new de,this._parser=null}get parser(){return this._parser||(this._parser=new Ne),this._parser}async parse(t,r){await Ne.init();let n=this.getParseTreeCache(t),s=n.get(r);if(s)return s.createReference();let o=await this.languageLoader.loadLanguage(t);if(this.parser.setLanguage(o),s=n.get(r),s)return s.createReference();let a=this.parser.parse(r);return s=new pe(a),n.put(r,s),s.createReference()}dispose(){this._parser&&(this.parser.delete(),this._parser=null);for(let t of this.caches.values())t.dispose()}getParseTreeCache(t){let r=this.caches.get(t);return r||(r=new _e(e.CACHE_SIZE_PER_LANGUAGE),this.caches.set(t,r)),r}},pe=class{constructor(t){this._tree=new he(t)}dispose(){this._tree.deref()}createReference(){return new fe(this._tree)}},fe=class{constructor(t){this._parseTree=t;this._parseTree.ref()}get tree(){return this._parseTree.tree}dispose(){this._parseTree.deref()}},he=class{constructor(t){this._tree=t;this._refCount=1}get tree(){if(this._refCount===0)throw new Error("Cannot access disposed RefCountedParseTree");return this._tree}ref(){if(this._refCount===0)throw new Error("Cannot ref disposed RefCountedParseTree");this._refCount++}deref(){if(this._refCount===0)throw new Error("Cannot deref disposed RefCountedParseTree");this._refCount--,this._refCount===0&&this._tree.delete()}};function ve(){le.INSTANCE.dispose()}function T(e,t){return le.INSTANCE.parse(e,t)}var fr=j(),ge=class{constructor(t){this.language=t;this.map=new Map}getQuery(t){return this.map.has(t)||this.map.set(t,this.language.query(t)),this.map.get(t)}},ye=class e{constructor(){this.map=new Map}static{this.INSTANCE=new e}getQuery(t,r){return this.map.has(t)||this.map.set(t,new ge(t)),this.map.get(t).getQuery(r)}};function R(e,t){let r=[];for(let n of e){let o=ye.INSTANCE.getQuery(t.tree.getLanguage(),n).matches(t);r.push(...o)}return r}var yr=j();function te(e,t){switch(t){case"python":case"csharp":return e.children.find(r=>r.type.match(/identifier/))?.text;case"go":{let r=e.children.find(s=>s.type.match(/identifier/));return r?r.text:e.children.find(s=>s.type.match(/spec/))?.children.find(s=>s.type.match(/identifier/))?.text}case"javascript":case"javascriptreact":case"typescript":case"typescriptreact":case"cpp":{let r=e.children.find(s=>s.type.match(/declarator/));return r?r.children.find(s=>s.type.match(/identifier/))?.text:e.children.find(s=>s.type.match(/identifier/))?.text}case"java":return e.children.find(n=>n.type==="identifier")?.text;case"ruby":return e.children.find(r=>r.type.match(/constant|identifier/))?.text;default:return e.children.find(r=>r.type.match(/identifier/))?.text}}function re(e,t){switch(t){case"typescript":case"tsx":case"javascript":return e.type.match(/definition|declaration|declarator|export_statement/);case"go":return e.type.match(/definition|declaration|declarator|var_spec/);case"cpp":return e.type.match(/definition|declaration|class_specifier/);case"ruby":return e.type.match(/module|class|method|assignment/);default:return e.type.match(/definition|declaration|declarator/)}}var Tr=j();function H(e,t,r,n=re){let s=[e.rootNode],o=[];for(;;){let a=s.map(_=>[_,x.intersectionSize(_,t)]).filter(([_,d])=>d>0).sort(([_,d],[l,c])=>c-d);if(a.length===0)return o.length===0?void 0:Re(o,([_,d],[l,c])=>d-c)[0];{let _=a.map(([d,l])=>{let c=x.len(d),m=Math.abs(x.len(t)-l),g=(l-m)/c;return[d,g]});o.push(..._.filter(([d,l])=>n(d,r))),s=[],s.push(..._.flatMap(([d,l])=>d.children))}}}var f=(()=>{function e(t,...r){return t.length===1?t[0]:t.reduce((n,s,o)=>`${n}${s}${r[o]||""}`,"")}return{typescript:e,javascript:e,python:e,go:e,ruby:e,csharp:e,cpp:e,java:e,rust:e}})();function A(e,t){return Object.fromEntries(e.map(r=>[r,t]))}var wt={javascript:[],typescript:[],tsx:[],python:[],csharp:[],go:[],java:[],ruby:[],cpp:[],rust:[]};function F(e){for(let t in e){let r=e[t];wt[t].push(...r)}return e}var Pe=F({...A(["javascript","typescript","tsx"],[`[
		...
		] @call_expression`]}),Ce=F({...A(["javascript","typescript","tsx"],["(class_declaration) @class_declaration"]),java:["(class_declaration) @class_declaration"],csharp:["(class_declaration) @class_declaration"],python:["(class_definition) @class_declaration"],cpp:["(class_specifier) @class_declaration"],ruby:["(class) @class_declaration"],go:[`(type_declaration
			(type_spec
				(type_identifier) @type_identifier)) @class_declaration`],rust:["(impl_item (type_identifier) @type_identifier) @class_declaration"]}),Ae=F({typescript:[`[
			...
			(identifier) @type_identifier) @type_declaration`]}),Fe=F({typescript:["(type_identifier) @type_identifier"],go:["(type_identifier) @type_identifier"],ruby:["(constant) @type_identifier"],csharp:[`[
			(base_list
			...
			(argument_list
				(identifier) @type_identifier)
		]`]}),Oe=F({...A(["javascript","typescript","tsx"],[`(new_expression
			...
			(assignment
				...
				right: (lambda) @body)
		] @function`,'(ERROR ("def" (identifier) (parameters))) @function'],...A(["javascript","typescript","tsx"],[`[
			(function_expression
				name: (identifier)? @identifier
		...
		] @function`]}),Rr=F({javascript:[f.javascript`((comment) @comment
			(#match? @comment "^\\\\/\\\\*\\\\*")) @docComment`],...A(["typescript","tsx"],[f.typescript`((comment) @comment
			...
			(string) @docComment)`]}),xe=F({javascript:[f.javascript`[
			(function_declaration
		...
		`],rust:[]}),Be={...A(["typescript","tsx"],["program","interface_declaration","class_declaration","function_declaration","function_expression","type_alias_declaration","method_definition"]),javascript:["program","class_declaration","function_declaration","function_expression","method_definition"],java:["program","class_declaration","interface_declaration","method_declaration"],cpp:["translation_unit","class_specifier","function_definition"],csharp:["compilation_unit","class_declaration","interface_declaration","method_declaration"],python:["module","class_definition","function_definition"],go:["source_file","type_declaration","function_declaration","method_declaration"],ruby:["program","method","class","method"],rust:["source_file","function_item","impl_item","let_declaration"]},De=F({typescript:[D("typescript")],tsx:[D("tsx")],javascript:[D("javascript")],java:[D("java")],cpp:[D("cpp")],csharp:[D("csharp")],python:[D("python")],go:[D("go")],ruby:[D("ruby")],rust:[D("rust")]}),We={...A(["typescript","tsx","javascript"],["for_in_statement","for_statement","if_statement","while_statement","do_statement","try_statement","switch_statement"]),java:["for_statement","enhanced_for_statement","if_statement","while_statement","do_statement","try_statement","switch_expression"],cpp:["for_statement","for_range_loop","if_statement","while_statement","do_statement","try_statement","switch_statement"],csharp:["for_statement","for_each_statement","if_statement","while_statement","do_statement","try_statement","switch_expression"],python:["for_statement","if_statement","while_statement","try_statement"],go:["for_statement","if_statement","type_switch_statement"],ruby:["while","for","if","case"],rust:["for_statement","if_statement","while_statement","loop_statement","match_expression"]},xt={...A(["typescript","tsx"],["lexical_declaration","expression_statement","public_field_definition"]),javascript:["call_expression","expression_statement","variable_declaration","public_field_definition"],java:["expression_statement","local_variable_declaration","field_declaration"],cpp:["field_declaration","expression_statement","declaration"],csharp:["field_declaration","expression_statement"],python:["expression_statement"],go:["short_var_declaration","call_expression"],ruby:["call","assignment"],rust:["expression_statement","let_declaration","use_declaration","assignment_expression","macro_definition","extern_crate_declaration"]},Tt={...A(["typescript","tsx"],["class_declaration","function_declaration","generator_function_declaration","interface_declaration","internal_module","method_definition","abstract_class_declaration"]),javascript:["class_declaration","function_declaration","generator_function_declaration","method_definition"],java:["class_declaration","constructor_declaration","enum_declaration","interface_declaration","method_declaration","module_declaration"],cpp:["class_specifier","function_definition","namespace_definition","struct_specifier"],csharp:["class_declaration","constructor_declaration","destructor_declaration","enum_declaration","interface_declaration","method_declaration","namespace_declaration","struct_declaration"],python:["function_definition","class_definition"],go:["function_declaration","method_declaration"],ruby:["class","method","module"],rust:["function_item","impl_item","mod_item","struct_item","trait_item","union_item"]},qe=F({typescript:[W("typescript")],tsx:[W("tsx")],javascript:[W("javascript")],java:[W("java")],cpp:[W("cpp")],csharp:[W("csharp")],python:[W("python")],go:[W("go")],rust:[W("rust")],ruby:[W("ruby")]});function D(e){return Be[e].map(t=>`(${t}) @scope`).join(`
`)}function W(e){return`[
		${Tt[e].map(r=>`(${r})`).join(`
`)}
	] @definition`}function ne(e,t){return Be[e].includes(t.type)||We[e].includes(t.type)}function Ge(e,t){return We[e].includes(t.type)}function Te(e,t){return xt[e].includes(t.type)}var Ve={...A(["typescript","tsx"],[f.typescript`[
			(expression_statement
				(call_expression
		...
		]`],ruby:[],csharp:[],cpp:[],rust:[]};var be=class{constructor(){this._cache=new J(5)}setCacheSize(t){this._cache=new J(t)}async getStructure(t,r){let n=`${t}:${r}`,s=this._cache.get(n);return s||(s=await this._getStructure(t,r),this._cache.put(n,s)),s}async _getStructure(t,r){let n=Ue[t];if(n.length===0)return;let s=await T(t,r);try{let o=R(n,s.tree.rootNode).flatMap(l=>l.captures).sort((l,c)=>x.compare(l.node,c.node)),a=[];for(let l of o)l.name.endsWith(".exclude_captures")&&a.push(x.ofSyntaxNode(l.node));let _=new Y(0,r.length,"root",[]),d=[_];for(let l=0;l<o.length;++l){let m=o[l].node;if(a.some(b=>x.isEqual(b,m)))continue;let h;do h=d.pop();while(h&&!x.doesContain(h,m));if(new Set(["export_statement","ambient_declaration"]).has(h.kind))h.kind=m.type,d.push(h);else{let b=m.type;(t==="typescript"||t==="tsx"||t==="javascript")&&b==="method_definition"&&m.namedChildren.some(S=>S.type==="property_identifier"&&S.text==="constructor")&&(b="constructor");let p=m.startIndex,E=m.previousSibling;if(E!==null){let O=r.substring(E.endIndex,m.startIndex).indexOf(`
`);O===-1?p=E.endIndex:p=E.endIndex+O+1}let y=m.endIndex,u=m.nextSibling;if(t==="typescript"&&u!==null&&(u.type===";"||u.type===",")&&(u=u.nextSibling),u!==null){let O=r.substring(m.endIndex,u.startIndex).indexOf(`
`);O!==-1&&(y=m.endIndex+O+1)}let w=new Y(p,y,b,[]);h.children.push(w),d.push(h,w)}}return _}catch(o){console.error(o instanceof Error?o:new Error(o))}finally{s.dispose()}}},je=new be;var Lr=j();async function He(e,t,r){let n=await T(e,t);try{let o=r.startIndex===r.endIndex?void 0:H(n.tree,r,e);if(o)return{nodeIdentifier:te(o,e),nodeToDocument:V.ofSyntaxNode(o),nodeSelectionBy:"matchingSelection"};let _=n.tree.rootNode.descendantForIndex(r.startIndex,r.endIndex),d=0;for(;!re(_,e)&&_.parent!==null;)_=_.parent,++d;return{nodeIdentifier:te(_,e),nodeToDocument:V.ofSyntaxNode(_),nodeSelectionBy:"expanding"}}finally{n.dispose()}}async function Ze(e,t,r){let n=await T(e,t);try{let s=n.tree.rootNode.descendantForIndex(r.startIndex,r.endIndex);if(s.type.match(/identifier/)&&(s.parent===null||re(s.parent,e))){let o=s.parent,a=o===null?void 0:{startIndex:o.startIndex,endIndex:o.endIndex};return{identifier:s.text,nodeRange:a}}}finally{n.dispose()}}function bt(e,t,r=0,n=e.length){let s=r,o=n;for(;s<o;){let a=Math.floor((s+o)/2);t(e[a])?s=a+1:o=a}return s-1}var $e=class e{constructor(t){this._array=t;this._findLastMonotonousLastIdx=0}static{this.assertInvariants=!1}findLastMonotonous(t){if(e.assertInvariants){if(this._prevFindLastPredicate){for(let n of this._array)if(this._prevFindLastPredicate(n)&&!t(n))throw new Error("MonotonousArray: current predicate must be weaker than (or equal to) the previous predicate.")}this._prevFindLastPredicate=t}let r=bt(this._array,t,this._findLastMonotonousLastIdx);return this._findLastMonotonousLastIdx=r+1,r===-1?void 0:this._array[r]}};function Je(e){let t=new Set;return r=>{let n=e(r);return t.has(n)?!1:(t.add(n),!0)}}var ze;(_=>{function e(d){return d<0}_.isLessThan=e;function t(d){return d<=0}_.isLessThanOrEqual=t;function r(d){return d>0}_.isGreaterThan=r;function n(d){return d===0}_.isNeitherLessOrGreaterThan=n,_.greaterThan=1,_.lessThan=-1,_.neitherLessOrGreaterThan=0})(ze||={});var Qe=class e{constructor(t){this.iterate=t}static{this.empty=new e(t=>{})}forEach(t){this.iterate(r=>(t(r),!0))}toArray(){let t=[];return this.iterate(r=>(t.push(r),!0)),t}filter(t){return new e(r=>this.iterate(n=>t(n)?r(n):!0))}map(t){return new e(r=>this.iterate(n=>r(t(n))))}some(t){let r=!1;return this.iterate(n=>(r=t(n),!r)),r}findFirst(t){let r;return this.iterate(n=>t(n)?(r=n,!1):!0),r}findLast(t){let r;return this.iterate(n=>(t(n)&&(r=n),!0)),r}findLastMaxBy(t){let r,n=!0;return this.iterate(s=>((n||ze.isGreaterThan(t(s,r)))&&(n=!1,r=s),!0)),r}};function se(e,t){if(!e)throw new Error(t?`Unexpected type, expected '${t}'`:"Unexpected type")}async function Ke(e,t,r){let n=await T(e,t);try{let s=R(xe[e],n.tree.rootNode).flatMap(({captures:_})=>_),o=new Map;for(let _ of s){let[d,l]=_.name.split(".");if(l!=="identifier")continue;let c=o.get(d)||[];c.push(_),o.set(d,c)}let a=null;for(let _ of s){let[d,l]=_.name.split(".");if(l!==void 0||!x.doesContain(_.node,r)||a!==null&&x.len(a.node)<x.len(_.node))continue;let c=o.get(d);se(c!==void 0,`must have seen identifier for symbol kind '${d}' (lang: ${e})`);let m=c.find(h=>x.doesContain(_.node,h.node));se(m!==void 0,`must have seen identifier for symbol '${d}' (lang: ${e})`),a={identifier:{name:m.node.text,range:x.ofSyntaxNode(m.node)},node:V.ofSyntaxNode(_.node)}}return a}catch(s){return console.error("getTestableNode: Unexpected error",s),null}finally{n.dispose()}}async function Ye(e,t){let r=await T(e,t);try{let n=R(xe[e],r.tree.rootNode).flatMap(({captures:a})=>a).filter(Je(a=>[a.node.startIndex,a.node.endIndex].toString())),s=new Map;for(let a of n){let[_,d]=a.name.split(".");if(d!=="identifier")continue;let l=s.get(_)||[];l.push(a),s.set(_,l)}let o=[];for(let a of n){if(a.name.includes("."))continue;let _=a.name,d=s.get(_);se(d!==void 0,`must have seen identifier for symbol kind '${_}' (lang: ${e})`);let l=d.find(c=>x.doesContain(a.node,c.node));se(l!==void 0,`must have seen identifier for symbol '${_}' (lang: ${e})`),o.push({identifier:{name:l.node.text,range:x.ofSyntaxNode(l.node)},node:V.ofSyntaxNode(a.node)})}return o}catch(n){return console.error("getTestableNodes: Unexpected error",n),null}finally{r.dispose()}}async function Xe(e,t){let r=await T(e,t);try{let s=R(Ve[e],r.tree.rootNode).flatMap(a=>a.captures).sort((a,_)=>a.node.endIndex-_.node.endIndex).filter(a=>a.name==="test");if(s.length===0)return null;let o=s[s.length-1].node;return{startIndex:o.startIndex,endIndex:o.endIndex}}finally{r.dispose()}}var xn=j();function Et(e,t){let r=De[e];return R(r,t)}function rt(e,t){let r=ke[e];return R(r,t)}function St(e,t){let r=Pe[e];return r?R(r,t):[]}function It(e,t){let r=Ce[e];return r?R(r,t):[]}function Mt(e,t){let r=Ae[e];return r?R(r,t):[]}function Rt(e,t){let r=Fe[e];return r?R(r,t):[]}function Nt(e,t){let r=Oe[e];return r?R(r,t):[]}function vt(e,t){let r=qe[e];return R(r,t)}async function Pt(e,t,r){let n=await T(e,t);try{return St(e,n.tree.rootNode).reduce((a,_)=>{let d=_.captures.find(l=>l.name==="call_expression").node;if(x.doIntersect(r,d)){let l,c;e==="ruby"&&(c=_.captures.find(m=>m.name==="symbol")?.node,l=c?.text?.slice(1)),c??=_.captures.find(m=>m.name==="identifier")?.node,l??=c?.text,a.push({identifier:l??"",text:d.text,startIndex:(c??d).startIndex,endIndex:(c??d).endIndex})}return a},[])}finally{n.dispose()}}async function Ct(e,t){let r=await T(e,t);try{return rt(e,r.tree.rootNode).map(o=>{let a=o.captures.find(d=>d.name==="function").node;return{identifier:o.captures.find(d=>d.name==="identifier")?.node.text??"",text:a.text,startIndex:a.startIndex,endIndex:a.endIndex}})}finally{r.dispose()}}async function At(e,t){let r=await T(e,t);try{return It(e,r.tree.rootNode).map(o=>{let a=o.captures.find(d=>d.name==="class_declaration").node;return{identifier:a?.children.find(d=>d.type==="type_identifier"||d.type==="identifier"||d.type==="constant")?.text??"",text:a.text,startIndex:a.startIndex,endIndex:a.endIndex}})}finally{r.dispose()}}async function Ft(e,t){let r=await T(e,t);try{return Mt(e,r.tree.rootNode).map(o=>{let a=o.captures.find(d=>d.name==="type_declaration").node,_=o.captures.find(d=>d.name==="type_identifier")?.node.text;return _||(_=a?.children.find(d=>d.type==="type_identifier")?.text),{identifier:_??"",text:a.text,startIndex:a.startIndex,endIndex:a.endIndex}})}finally{r.dispose()}}async function Ot(e,t,r){let n=await T(e,t);try{return Rt(e,n.tree.rootNode).reduce((a,_)=>{let d=_.captures.find(l=>l.name==="type_identifier").node;return x.doIntersect(r,d)&&a.push({identifier:d.text,text:d.text,startIndex:d.startIndex,endIndex:d.endIndex}),a},[])}finally{n.dispose()}}async function kt(e,t,r){let n=await T(e,t);try{return Nt(e,n.tree.rootNode).reduce((a,_)=>{let d=_.captures.find(l=>l.name==="new_expression").node;return x.doIntersect(r,d)&&a.push({identifier:d.text,text:d.text,startIndex:d.startIndex,endIndex:d.endIndex}),a},[])}finally{n.dispose()}}async function Lt(e,t,r){let n=await T(e,t);try{let s=Le[e];return R(s,n.tree.rootNode).reduce((_,d)=>{let l=d.captures.find(c=>c.name==="symbol").node;return x.doIntersect(r,l)&&_.push({identifier:l.text,text:l.text,startIndex:l.startIndex,endIndex:l.endIndex}),_},[])}finally{n.dispose()}}async function Ut(e,t){let r=await T(e,t);try{let n=vt(e,r.tree.rootNode);return jt(e,n,r.tree.rootNode)}finally{r.dispose()}}async function Bt(e,t){let r=await T(e,t);try{return rt(e,r.tree.rootNode).map(o=>{let a=o.captures.find(_=>_.name==="body").node;return{startIndex:a.startIndex,endIndex:a.endIndex}})}finally{r.dispose()}}async function Dt(e,t,r){let n=await T(e,t);try{let s=Et(e,n.tree.rootNode),o;for(let a of s){let _=a.captures[0].node,d=G.ofSyntaxNode(_);if(G.doesContain(d,r)&&(o=_),U.isBefore(r.endPosition,d.startPosition))break}if(o)return G.ofSyntaxNode(o);throw new Error("No parent node found")}finally{n.dispose()}}async function Wt(e,t,r,n){let s=await T(e,t);try{let o=s.tree.rootNode.descendantForPosition(r.startPosition,r.endPosition),a={startPosition:o.startPosition,endPosition:o.endPosition},_=st(e,o,n,r,!0);return G.equals(a,_)?nt(e,o):_}finally{s.dispose()}}function nt(e,t){let r=t.parent,n={startPosition:t.startPosition,endPosition:t.endPosition};if(ne(e,t)||!r)return n;let{filteredRanges:s,indexOfInterest:o}=ot(e,r.children,n,!1);if(o-1>=0&&o+1<=s.length-1){let a=s[o-1],_=s[o+1];return{startPosition:a.startPosition,endPosition:_.endPosition}}return nt(e,r)}function st(e,t,r,n,s){let o=t.children;if(t.endPosition.row-t.startPosition.row+1<=r){let _=ne(e,t)?{startPosition:t.startPosition,endPosition:t.endPosition}:tt(e,o,r,n,s),d=t.parent;return d?st(e,d,r,_,!1):_}return tt(e,o,r,n,s)}function et(e,t){return t.endPosition.row-e.startPosition.row+1}function tt(e,t,r,n,s){if(t.length===0)return n;let{filteredRanges:o,indexOfInterest:a}=ot(e,t,n,s),_=0,d=o.length-1,l=o[_],c=o[d];for(;et(l,c)>r&&_!==d;)a-_<d-a?(d--,c=o[d]):(_++,l=o[_]);return et(l,c)<=r?{startPosition:l.startPosition,endPosition:c.endPosition}:n}function ot(e,t,r,n){let s,o;if(n?(s=t.filter(a=>ne(e,a)||Te(e,a)),o=Me(s,r,(a,_)=>U.isBefore(a.startPosition,_.startPosition)),s.splice(o,0,r)):(s=t.filter(a=>G.doesContain(a,r)||ne(e,a)||Te(e,a)),o=s.findIndex(a=>G.doesContain(a,r))),o===-1)throw new Error("Valid index not found");return{filteredRanges:s,indexOfInterest:o}}async function qt(e,t,r){let n=[],a=(await T(e,t)).tree.rootNode.descendantForIndex(r.startIndex,r.endIndex);for(;a!==null;)Ge(e,a)&&n.push({startIndex:a.startIndex,endIndex:a.endIndex}),a=a.parent;return n}async function Gt(e,t,r){let n=await T(e,t);try{let s=r.startIndex===r.endIndex;if(s)return;let o=s?void 0:H(n.tree,r,e),a=s?void 0:H(n.tree,r,e,Vt);if(a&&o)return{nodeIdentifier:te(o,e),nodeToExplain:V.ofSyntaxNode(a)}}finally{n.dispose()}}function Vt(e,t){return e.type.match(/definition/)}function jt(e,t,r){let n;switch(e){case"python":n=Qt(t);break;case"ruby":n=$t(t);break;default:{n=Ht(t,e);break}}return new ae(n,B.ofSyntaxNode(r))}function Ht(e,t){let r=new Map;return e.forEach(n=>{let o=n.captures.find(_=>_.name==="definition")?.node,a=o?.childForFieldName("body");if(o&&a){let _;switch(t){case"typescript":case"javascript":{let{definition:l,comments:c}=Jt(o);o=l,_=c;break}case"java":case"rust":_=zt(o);break;default:{_=oe(o);break}}r.get(o.id)||r.set(o.id,{mainBlock:B.ofSyntaxNode(o),detailBlocks:{comments:_.map(l=>B.ofSyntaxNode(l)),body:B.ofSyntaxNode(a)}})}}),Array.from(r.values())}function Zt(e){if(!(e.length<2))for(let t=1;t<e.length;t++){let r=e[t];if(!r.type.includes("parameters"))return r}}function $t(e){let t=new Map;return e.forEach(r=>{let s=r.captures.find(o=>o.name==="definition")?.node;if(s){let o=s.namedChildren,a=Zt(o);if(a){let _=o[o.length-1],d=s.text.substring(a.startIndex-s.startIndex,_.endIndex-s.startIndex),l=oe(s);t.get(s.id)||t.set(s.id,{mainBlock:B.ofSyntaxNode(s),detailBlocks:{comments:l.map(m=>B.ofSyntaxNode(m)),body:{range:{startPosition:{row:a.startPosition.row,column:a.startPosition.column},endPosition:{row:_.endPosition.row,column:_.endPosition.column}},startIndex:a.startIndex,text:d,endIndex:_.endIndex}}})}}}),Array.from(t.values())}function Qt(e){let t=new Map;return e.forEach(r=>{let s=r.captures.find(a=>a.name==="definition")?.node,o=s?.childForFieldName("body");if(s&&o){let a=Yt(o),_=Kt(s);t.set(s.id,{mainBlock:B.ofSyntaxNode(s),detailBlocks:{docstring:a?B.ofSyntaxNode(a):void 0,decorator:_?B.ofSyntaxNode(_):void 0,body:B.ofSyntaxNode(o)}});return}}),Array.from(t.values())}function oe(e,t=["comment"]){let r=[],n=e.previousNamedSibling;for(;n&&t.some(s=>s===n?.type);)r.push(n),n=n.previousNamedSibling;return r.reverse()}function Jt(e){let t=e.parent;return t?.type==="export_statement"?{definition:t,comments:oe(t)}:{definition:e,comments:oe(e)}}function zt(e){return oe(e,["block_comment","line_comment"])}function Kt(e){let t=e.previousNamedSibling;return t?.type==="decorator"?t:void 0}function Yt(e){let t=e.firstChild;if(!t||t.type!=="expression_statement")return;let r=t.firstChild;return r?.type==="string"?r:void 0}function Xt(e,t){return je.getStructure(e,t)}function er(){let e=it.parentPort;if(!e)throw new Error("This module should only be used in a worker thread.");e.on("message",async({id:t,fn:r,args:n})=>{try{let s=await Ee[r](...n);e.postMessage({id:t,res:s})}catch(s){e.postMessage({id:t,err:s})}})}er();
